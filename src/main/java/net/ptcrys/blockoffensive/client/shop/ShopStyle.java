...
public final class ShopStyle {
    ...
    public void apply(View view, ModernScreen.Node node, float scale, boolean changed, int accent) {
        ...
        int color = node.kind() == ModernScreen.Kind.MUTED ? 0xFF717171 : node.kind() == ModernScreen.Kind.ACCENT ? accent : 0xFFD1D1D1;
        float size = 14;
        ...
        if (key.equals("money")) {
        ...
        } else if (key.startsWith("category.name.")) {
            size = 26;
            gravity = Gravity.CENTER;
            color = node.selected() ? accent : 0xFFD3D3D3;
        } else if (key.startsWith("category.number.")) {
            size = 16;
            color = node.selected() ? accent : 0xFFBDBDBD;
            ...
            bold = true;
            gravity = Gravity.RIGHT | Gravity.CENTER_VERTICAL;
        } else if (key.equals("owned")) {
            color = accent;
            size = 13;
            bold = true;
            gravity = Gravity.CENTER;
        } else if (key.equals("drops.empty")) {
            color = 0xFF393939;
            size = 80;
            bold = true;
            gravity = Gravity.CENTER;
        } else if (key.equals("controls") || key.equals("refundAll") || key.equals("close")) {
            color = accent;
            size = 20;
            bold = true;
            gravity = Gravity.CENTER;
    ...
    private static final class MeshPanel extends Drawable {
        ...
        MeshPanel(float scale) {
            step = Math.max(3, 4 * scale);
        }
