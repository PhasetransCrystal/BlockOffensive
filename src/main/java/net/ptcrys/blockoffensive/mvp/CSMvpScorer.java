...
public final class CSMvpScorer {
    ...
    private static int score(CSMvpContribution contribution) {
        ...
        return objectiveScore + contribution.kills() * 3 + contribution.assists() + contribution.headshotKills() + Math.round(directDamage / 50.0F) + Math.round(contribution.incendiaryDamage() / 25.0F) + Math.round(contribution.explosiveDamage() / 45.0F);
    }
    ...
    private static String reasonKey(CSMvpContribution contribution) {
        ...
        float utilityDamage = contribution.incendiaryDamage() + contribution.explosiveDamage();
        float directDamage = Math.max(0.0F, contribution.damage() - utilityDamage);
        int combatScore = contribution.kills() * 3 + contribution.assists() + contribution.headshotKills() + Math.round(directDamage / 50.0F);
