package com.ptcrys.blockoffensive.util;
...
public final class PlayerColorAssignments {
    ...
    public boolean reconcile(Collection<UUID> roster) {
        ...
        for (UUID id : roster.stream().sorted().toList()) {
            if (!colors.containsKey(id)) { assign(id); changed = true; }
        }
        return changed;
    }
    ...
    public int get(UUID id) { return colors.getOrDefault(id, 0xFFFFFFFF); }
    public boolean remove(UUID id) { return colors.remove(id) != null; }
    public Map<UUID, Integer> snapshot() { return new LinkedHashMap<>(colors); }
    public void replace(Map<UUID, Integer> snapshot) { colors.clear(); colors.putAll(snapshot); }
