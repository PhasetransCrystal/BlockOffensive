...
public final class ShopDropGameTests {
    ...
    public static void shopResultChannelIsRegistered(GameTestHelper helper) {
        helper.assertTrue(com.ptcrys.fpsmatch.common.packet.register.NetworkPacketRegister.getChannelFromCache(
                com.ptcrys.fpsmatch.common.packet.shop.ShopActionResultS2CPacket.class) != null,
    ...
    public static void mixedDropsAndNativePickup(GameTestHelper helper) {
        ...
        vanilla.setNoPickUpDelay(); vanilla.setNoGravity(true);
        ...
        owned.setTarget(UUID.randomUUID()); owned.setNoPickUpDelay(); owned.setNoGravity(true);
        ...
        match.setPos(player.getX() + 4, player.getY(), player.getZ()); match.setNoGravity(true);
        helper.getLevel().addFreshEntity(match);
        helper.assertTrue(ShopDropPickupService.pickupDetailed(player, match.getUUID(), 8, s -> true, true).reason()
                == ShopDropPickupService.FailureReason.PICKUP_DELAY, "match pickup delay must be enforced");
    ...
    public static void fullInventoryPreservesMatchDrop(GameTestHelper helper) {
        ...
        match.setPos(player.getX() + 2, player.getY(), player.getZ()); match.setNoGravity(true);
        helper.getLevel().addFreshEntity(match);
        helper.runAfterDelay(22, () -> {
            ...
            match.discard(); player.getInventory().clearContent();
            helper.succeed();
        });
    }
