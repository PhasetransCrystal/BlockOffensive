...
public final class ShopPlayerPreview extends RemotePlayer {
    ...
    @Override public ResourceLocation getSkinTextureLocation() { return source.getSkinTextureLocation(); }
    @Override public String getModelName() { return source.getModelName(); }
    @Override public ResourceLocation getCloakTextureLocation() { return null; }
    ...
    public static ShopPreviewSelection.Category category(ItemStack stack) {
        ...
        if (GunCompatManager.isGun(stack)) {
            return GunCompatManager.findProvider(stack).getGunTabType(stack) == GunTabTypeEnum.PISTOL
                    ? ShopPreviewSelection.Category.PISTOL : ShopPreviewSelection.Category.PRIMARY;
        }
    ...
    public void render(GuiGraphics graphics, ItemStack held, int x, int y, int scale) {
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            ItemStack desired = slot == EquipmentSlot.MAINHAND ? held
                    : slot == EquipmentSlot.OFFHAND ? ItemStack.EMPTY : source.getItemBySlot(slot);
    ...
    public void applyPresentationPose(PlayerModel<?> model) {
        ...
        if (!nativeGun && IClientItemExtensions.of(stack).getArmPose(this, InteractionHand.MAIN_HAND, stack) == null) {
            if (kind == ShopPreviewSelection.Category.PRIMARY || kind == ShopPreviewSelection.Category.PISTOL) {
                main.xRot = -1.3f; main.yRot = -0.25f * side;
                other.xRot = -1.35f; other.yRot = 0.6f * side;
            } else {
