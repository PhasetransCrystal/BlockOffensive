...
public final class CSKillFeedGeometry {
    ...
    public static StackGeometry stack(
        ...
        int x = isRightAligned(normalizedPosition)
                ? screenWidth - horizontalMargin - width
                : horizontalMargin;

        int maxRows = Math.max(
                1,
                (screenHeight - CSHudSafeAreaLayouts.KILL_FEED_MARGIN * 2)
                        / CSHudSafeAreaLayouts.KILL_FEED_ROW_HEIGHT
        );
        ...
        int y = isBottomAligned(normalizedPosition)
