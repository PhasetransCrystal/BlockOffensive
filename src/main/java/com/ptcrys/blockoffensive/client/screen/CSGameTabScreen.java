...
import com.ptcrys.blockoffensive.client.screen.hud.CSGameTabRenderer;
...
public final class CSGameTabScreen extends TabScreen {
    ...
    public static void onMouseButton(InputEvent.MouseButton.Pre event) {
        ...
        if (event.getButton() != GLFW.GLFW_MOUSE_BUTTON_RIGHT || event.getAction() != GLFW.GLFW_PRESS
                || mc.screen != null || mc.player == null || mc.level == null
                || !mc.options.keyPlayerList.isDown() || !FPSMGameHudManager.shouldRender()
                || !FPSMClient.getGlobalData().isCurrentGameType("cs")) return;
    ...
    public void tick() {
        ...
        if (minecraft.player == null || minecraft.level == null || !minecraft.isWindowActive()
                || !FPSMGameHudManager.shouldRender() || !FPSMClient.getGlobalData().isCurrentGameType("cs")
                || !isHeld()) {
            onClose();
        }
    }
    ...
    public boolean keyReleased(int keyCode, int scanCode, int modifiers) {
        boolean released = heldKey.getType() == InputConstants.Type.SCANCODE
                ? heldKey.getValue() == scanCode : heldKey.equals(InputConstants.getKey(keyCode, scanCode));
        if (released) {
            onClose();
            return true;
        }
