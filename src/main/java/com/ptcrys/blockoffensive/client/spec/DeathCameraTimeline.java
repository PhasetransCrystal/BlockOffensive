...
public final class DeathCameraTimeline {
    ...
    public SequenceClock clock() { return clock; }
    private boolean active;

    public void start() { clock.reset(); active = true; }
    public void reset() { clock.reset(); active = false; }
    public void tick() { if (active) clock.tick(); }
    public boolean active() { return active; }
    public boolean showInformation() { return active && clock.ticks() < PRESENTATION_TICKS; }
    public boolean waitingForTarget() { return active && clock.ticks() >= PRESENTATION_TICKS; }
    public boolean targetWaitExpired() { return active && clock.ticks() >= PRESENTATION_TICKS + TARGET_WAIT_TICKS; }
    public boolean shouldRequestTarget() { return waitingForTarget() && (clock.ticks() - PRESENTATION_TICKS) % 5 == 0; }
