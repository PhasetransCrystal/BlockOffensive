...
public class MvpMusicUploadC2SPacket {
    ...
    private boolean isValidChunk() {
        return data != null && data.length > 0
                && totalChunks > 0 && totalChunks <= MvpMusicServerStore.MAX_CHUNKS
                && chunkIndex >= 0 && chunkIndex < totalChunks
                && data.length <= MvpMusicServerStore.MAX_CHUNK_BYTES;
    }

    private boolean isValidDisplayName() {
        return !displayName.isBlank() && displayName.length() <= MAX_DISPLAY_NAME_LENGTH
                && displayName.codePoints().noneMatch(Character::isISOControl);
    }
}